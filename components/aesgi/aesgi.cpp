...
void Aesgi::loop() {
  ...
  if (this->state_ == STATE_POLL_CHECKED) {
    std::string fc;
    char tmp[AESGI_READ_BUFFER_LENGTH];
  ...
  if (this->state_ == STATE_COMMAND || this->state_ == STATE_POLL) {
    while (this->available()) {
      ...
      if (this->read_pos_ == AESGI_READ_BUFFER_LENGTH) {
        this->read_pos_ = 0;
        this->empty_uart_buffer_();
      }
  ...
  if (this->state_ == STATE_COMMAND) {
    ...
    if (millis() - this->command_start_millis_ > esphome::aesgi::Aesgi::COMMAND_TIMEOUT) {
...
uint8_t Aesgi::check_incoming_length_(uint8_t length) {
...
uint8_t Aesgi::send_next_command_() {
...
void Aesgi::queue_command_(const char *command, uint8_t length) {
...
void Aesgi::add_polling_command_(const char *command, ENUMPollingCommand polling_command) {
  for (auto &used_polling_command : this->used_polling_commands_) {
...
void Aesgi::publish_state_(switch_::Switch *obj, const bool &state) {
  if (obj == nullptr)
    return;
